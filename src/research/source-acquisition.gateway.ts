...
function errorCode(error: unknown): string { const message = String(error instanceof Error ? error.message : error).toLowerCase(); const status = message.match(/http\s+(\d{3})/)?.[1]; if (status) return `http_${status}`; if (message.includes("timeout")) return "timeout"; if (message.includes("unsupported feed") || message.includes("expected rss")) return "invalid_feed"; if (message.includes("exceeds") && message.includes("bytes")) return "response_too_large"; if (message.includes("private") || message.includes("public http")) return "unsafe_url"; return "fetch_failed"; }
...
function gdeltQuery(topicCodes: string[], customTopics: string[]): string { const terms = [...customTopics, ...topicCodes.flatMap((code) => ({ ai:["artificial intelligence"], science:["scientific discovery", "research breakthrough"], technology:["technology", "innovation", "engineering"] }[code] ?? []))].map((v) => String(v).normalize("NFKC").replace(/[^\p{L}\p{N}\p{M}\s-]/gu, " ").trim().replace(/\s+/g, " ").slice(0, 60)).filter((v) => v.length >= 2).map((v) => v.includes(" ") ? `"${v}"` : v); return terms.length ? `(${[...new Set(terms)].slice(0, 16).join(" OR ")})` : "breaking news"; }
...
export class SourceAcquisitionGateway implements SourceAcquisition {
  ...
  private async safeFetch(value: string | URL, init: RequestInit, maxRedirects: number): Promise<{ response: Response; finalUrl: URL }> { let url = this.assertUrl(value); for (let redirects = 0;; redirects += 1) { const host = url.hostname.replace(/^\[|\]$/g, ""); if (!isIP(host)) { const addresses = await this.dns.lookup(host); if (!addresses.length) throw new Error(`DNS returned no addresses for ${host}`); addresses.forEach(({ address }) => this.assertIp(address)); } const response = await this.transport.fetch(url, { ...init, redirect: "manual" }); if (!REDIRECTS.has(response.status)) return { response, finalUrl: url }; if (redirects >= maxRedirects) throw new Error(`Request exceeded ${maxRedirects} redirects`); const location = response.headers.get("location"); if (!location) throw new Error(`Redirect response ${response.status} has no location`); await response.body?.cancel(); url = this.assertUrl(new URL(location, url)); } }
