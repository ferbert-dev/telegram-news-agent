...
From v2.0.0 production runs the typed NestJS runtime; the legacy JavaScript
runtime ships in the same image as the rollback. The runtime is a standalone
Nest application context -- no HTTP server -- composed from 31 modules. The
picture of one run through it is at the top of the [README](../README.md).
